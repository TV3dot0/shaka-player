...
shaka.polyfill.PatchedMediaKeysApple.MediaKeySystemAccess = class {
   ...
   * @return {?MediaKeySystemConfiguration} A matching config we can
   *   or
...
shaka.polyfill.PatchedMediaKeysApple.MediaKeySession =
    class extends shaka.util.FakeEventTarget {
      ...
      constructor(nativeMediaKeys, sessionType) {
        ...
        /**
         * The native MediaKeySession, which will
         * be created in generateRequest.
      ...
      generateRequest(initDataType, initData) {
        ...
        try {
          // This EME spec version requires a MIME content type
          // as the 1st param to
          // createSession, but doesn't seem to matter what the value is.
          // It also only accepts Uint8Array, not ArrayBuffer,
          // so explicitly make
      ...
      remove() {
        ...
        return Promise.reject(new Error(
            'MediaKeySession.remove is only applicable ' +
      ...
      onWebkitKeyMessage_(event) {
        ...
        // We can now resolve this.generateRequestPromise,
