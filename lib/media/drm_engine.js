...
shaka.media.DrmEngine = function(playerInterface) {
  ...
  /** @private {!Map.<string,
  *             ArrayBuffer>} */
  this.persistedSessions_ = new Map();
  ...
  const noop = () => {};
  this.allSessionsLoaded_
      .then((_) => console.log('[drm] all sessions are loaded'))
      .catch(noop);
};
 ...
 * @param {Map.<string,ArrayBuffer>=} persistedSessions
 * @return {!Promise}
 */
shaka.media.DrmEngine.prototype.initForPlayback = function(
    variants, offlineSessionIds, persistedSessions = new Map()) {
  this.offlineSessionIds_ = offlineSessionIds;
  this.persistedSessions_ = persistedSessions || new Map();
  this.usePersistentLicenses_ =
      // shaka.util.Platform.isApple() && !shaka.util.Platform.isMobile() ||
      shaka.util.Platform.isChrome();
      // offlineSessionIds.length > 0;
  const licenseType = this.usePersistentLicenses_? 'persistent':'temp';
  console.log(`[drm] requesting ${licenseType} licenses`);
...
shaka.media.DrmEngine.prototype.init_ = function(variants) {
  ...
  console.log('[drm] configsByKeySystem', configsByKeySystem);
  const p = this.queryMediaKeys_(configsByKeySystem);
  console.log('[drm] queryMediaKeys', p);
...
shaka.media.DrmEngine.prototype.attach = function(video) {
  ...
  return Promise.all([setMediaKeys, setServerCertificate]).then(() => {
    ...
    this.createOrLoad()
        .then((_) => console.log('[drm] session(s) created / loaded'));
    if (!this.currentDrmInfo_.initData.length &&
      ...
      console.log('[drm] no initData or offline -> new callback',);
      const cb = (e) => {
        console.log('[drm] encrypted');
...
shaka.media.DrmEngine.prototype.createOrLoad = function() {
  ...
  if (this.offlineSessionIds_.length == 0) {
    console.log('[drm] no offline sessions');
    const pendingTemps = initDatas.map((initDataOverride) => {
      return this.createTemporarySession_(initDataOverride.initDataType,
          initDataOverride.initData);
    });
    Promise.all(pendingTemps);
  } else {
    console.log('[drm] found offline sessions');
    const pendingOffs = this.offlineSessionIds_.map((sessionId) => {
      return this.loadOfflineSession_(sessionId);
    });
    Promise.all(pendingOffs);
  }
    // Load each session.

  Array.from(this.activeSessions_.entries())
    .forEach((e) => {
      // console.log('[drm] init', 'ms', e[0], 'id', e[1])
    });
...
shaka.media.DrmEngine.prototype.newInitData = function(initDataType, initData) {
  ...
  const sessions = this.activeSessions_.entries();
  for (const session of sessions) {
    if (Uint8ArrayUtils.equal(initData, session[1].initData)) {
      // console.log('Duplicate init data.', session[0], session[1].loaded);
      return;
    }
  }
  if (initData && initData != null) {
    // console.log('[drm] new initData', initDataType, initData);
    this.createTemporarySession_(initDataType, initData);
  } else {
    // console.log('[drm] initData null');
  }
};
...
/**
 * @return {!Map.<string,ArrayBuffer>}
 */
shaka.media.DrmEngine.prototype.getPersistedSessions = function() {
  return this.persistedSessions_;
};
...
shaka.media.DrmEngine.prototype.queryMediaKeys_ = function(configsByKeySystem) {
  if (configsByKeySystem.size == 1 && configsByKeySystem.has('')) {
    console.log('[drm] error no recognized key systems');
    return Promise.reject(new shaka.util.Error(
        shaka.util.Error.Severity.CRITICAL,
        shaka.util.Error.Category.DRM,
  ...
  [true, false].forEach(function(shouldHaveLicenseServer) {
    configsByKeySystem.forEach((config, keySystem) => {
      // console.log("[drm] testing keySystem", config, keySystem);
      ...
      p = p.catch(function() {
        if (this.isDestroying_) { return; }
        console.log('[drm] requesting mks accss');
  ...
  p = p.catch(() => {
    console.log('[drm] error requested ks unavailable');
    return Promise.reject(new shaka.util.Error(
        shaka.util.Error.Severity.CRITICAL,
        shaka.util.Error.Category.DRM,
  ...
  p = p.then(function(mediaKeySystemAccess) {
    ...
    console.log('[drm] ms config', realConfig);
    ...
    console.log('[drm] supported types', this.supportedTypes_);
    ...
    console.log('[drm] drmInfo', this.currentDrmInfo_);
    ...
    if (this.isDestroying_) { return Promise.reject(); }
    console.log('[drm] Created MediaKeys object for key system',
                   this.currentDrmInfo_.keySystem, mediaKeys);
...
shaka.media.DrmEngine.prototype.loadOfflineSession_ = function(sessionId) {
  ...
  let session;
  try {
    console.log('[drm] Attempting to load an offline session',
        sessionId,
        new Map(this.persistedSessions_));
    session = this.mediaKeys_.createSession('persistent-license');
  } catch (exception) {
    console.log('[drm] failed to create session');
  ...
  const savedInitData = this.persistedSessions_.get(sessionId);
  const initData = savedInitData ? new Uint8Array(savedInitData) : null;
  console.log('[drm] created offline session', sessionId);
  const metadata = {
    initData,
    loaded: false,
  ...
  return session.load(sessionId).then(function(present) {
    if (this.isDestroying_) { return Promise.reject(); }
    console.log('[drm] Loaded offline session', session, present);

    if (!present) {
      this.activeSessions_.delete(session);
      if (this.usePersistentLicenses_) {
        this.persistedSessions_.delete(sessionId);
      }
    ...
    // console.log('[drm] keys:', keyStatusMap.size);
    keyStatusMap.forEach((status, keyId) => console.log(status, keyId));

    // TODO: We should get a key status change event.  Remove once Chrome CDM
    // is fixed.
    metadata.loaded = true;
    if (this.areAllSessionsLoaded_()) {
      console.log('[drm] all sessions loaded');
      this.allSessionsLoaded_.resolve();
    }
    // try to wake up loaded session
    /*
    session.generateRequest('cenc', initData.buffer)
        .catch((error) => {
          if (this.isDestroying_) { return; }
    ...
    */
    ...
    console.log('[drm] problem loading session', error);
    if (this.isDestroying_) { return; }
...
shaka.media.DrmEngine.prototype.createTemporarySession_ =
  ...
  try {
    if (this.usePersistentLicenses_) {
      console.log('[drm] Creating new persistent session');
      session = this.mediaKeys_.createSession('persistent-license');
    } else {
      console.log('[drm] Creating new temporary session');
  ...
  session.generateRequest(initDataType, initData.buffer)
      .then((_) => {
        if (this.usePersistentLicenses_) {
          const sessionId = session.sessionId || '';
          console.log('[drm] persisting session', sessionId);
          this.persistedSessions_.set(sessionId, initData.buffer);
        }
        return _;
      })
      .catch((error) => {
...
shaka.media.DrmEngine.prototype.onSessionMessage_ = function(event) {
  console.log('[drm] session message, delayed?', this.delayLicenseRequest_());
...
shaka.media.DrmEngine.prototype.sendLicenseRequest_ = function(event) {
  ...
  console.log(
      '[drm] Sending license request for session', session.sessionId, 'of type',
      event.messageType);
  ...
  this.playerInterface_.netEngine.request(requestType, request).promise
      .then(function(response) {
        ...
        console.log('[drm] Updating session', session.sessionId);
...
shaka.media.DrmEngine.prototype.onKeyStatusesChange_ = function(event) {
  ...
  console.log('[drm] Key status changed for session', session.sessionId);
  ...
  keyStatusMap.forEach(function(status, keyId) {
    ...
    // console.log('[drm] key', status, keyId);
  ...
  if (msUntilExpiration < 0 || (hasExpiredKeys && msUntilExpiration < 1000)) {
    ...
    if (found && !found.updatePromise) {
      ...
      this.activeSessions_.delete(session);
      this.persistedSessions_.delete(session.sessionId);
...
shaka.media.DrmEngine.prototype.onPlay_ = function() {
  // console.log('[drm] processing play', this.mediaKeyMessageEvents_.length);
