...
shaka.media.DrmEngine.prototype.initForPlayback = function(
    variants, offlineSessionIds, persistedSessions) {
  this.offlineSessionIds_ = offlineSessionIds;
  this.persistedSessions_ = persistedSessions;
 ...
 */
shaka.media.DrmEngine.prototype.getPersistentSessions = function() {
...
shaka.media.DrmEngine.prototype.loadOfflineSession_ = function(sessionId) {
  ...
  try {
    ...
    session = /** @type {!MediaKeySession} */ this.mediaKeys_.createSession('persistent-license');
  } catch (exception) {
  ...
  const metadata = {
    initData: new Uint8Array(this.persistedSessions_.get(sessionId)),
    loaded: false,
    oldExpiration: Infinity,
    updatePromise: null,
  };
  this.activeSessions_.set(session, metadata.initData.buffer);
