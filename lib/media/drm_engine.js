...
shaka.media.DrmEngine.prototype.initForPlayback = function(
  ...
  shaka.log.info('[shaka] requested persistent licenses');
  this.usePersistentLicenses_ = true; // offlineSessionIds.length > 0;
...
shaka.media.DrmEngine.prototype.createOrLoad = function() {
  ...
  return Promise.all(pendingOffs)
    .then((_) => Array.from(this.activeSessions_.entries())
      .forEach((e) => console.log('[drm] offs', 'ms', e[0], 'id', e[1])))
    .then((_) => Promise.all(initDatas.map((initDataOverride) => {
      return this.createTemporarySession_(initDataOverride.initDataType,
          initDataOverride.initData);
    })))
    .then((_) => Array.from(this.activeSessions_.entries())
      .forEach((e) => console.log('[drm] temps', 'ms', e[0], 'id', e[1])))
    .then((_) => this.allSessionsLoaded_);
};
...
shaka.media.DrmEngine.prototype.loadOfflineSession_ = function(sessionId) {
  ...
  const initData = savedInitData ?
      shaka.util.Uint8ArrayUtils.fromBase64(savedInitData) :
      null;
  console.log('created session', sessionId, savedInitData, initData);
  const metadata = {
    initData,
    loaded: false,
