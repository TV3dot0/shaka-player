...
shaka.Player.prototype.onInitializeDrm_ = async function(has, wants) {
  ...
  console.log('[player] drm config',
      JSON.stringify(this.config_.drm));
  this.drmEngine_.configure(this.config_.drm);

  this.manifest_.offlineSessionIds = this.config_.drm.offlineSessionIds || [];

  this.dispatchEvent(new shaka.util.FakeEvent(
      /* name= */ 'onplayerinit',
      /* data= */ {'manifest': this.manifest_, 'config': this.config_.drm}));

  console.log('[player] drm cfg', Object.assign({}, this.config_.drm));
  console.log('[player] off ids', this.manifest_.offlineSessionIds.slice());

  const persistedSessions64 = /** @type {!Object.<string,string>} */(this.config_.drm.persistedSessions);
  const persistedSessions =
      /** @type {!Object.<string,ArrayBuffer>} */(
      Object.entries(persistedSessions64)
          .reduce((ss, s) => {
            ss[s[0]] = shaka.util.Uint8ArrayUtils.fromBase64(/** @type {!string} */(s[1]));
            return ss;
          }, {})
  );
  console.log('[player] sess obj', persistedSessions);

  const persistedSessionsMap =
      /** @type {!Map.<string,ArrayBuffer>} */(
      new Map(Object.entries(persistedSessions)));
  console.log('[player] sessions', new Map(persistedSessionsMap));

  await this.drmEngine_.initForPlayback(
      shaka.util.Periods.getAllVariantsFrom(this.manifest_.periods),
      this.manifest_.offlineSessionIds,
      persistedSessionsMap);
...
shaka.Player.prototype.configure = function(config, value) {
  ...
  console.log('[player] configure drm0', JSON.stringify(config));
  this.applyConfig_();
  console.log('[player] configure drm1', JSON.stringify(this.config_.drm));
  return ret;
};
...
shaka.Player.prototype.applyConfig_ = function() {
  ...
  console.log('[player] configure drm2', JSON.stringify(this.config_.drm));
  if (this.streamingEngine_) {
    this.streamingEngine_.configure(this.config_.streaming);
...
shaka.Player.prototype.getPersistedSessions = function() {
  return this.drmEngine_ ? this.drmEngine_.getPersistedSessions() : new Map();
};
