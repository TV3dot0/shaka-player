...
shaka.Player.prototype.onInitializeDrm_ = async function(has, wants) {
  ...
  this.manifest_.persistedSessions =
      new Map(Object.entries(this.config_.drm.persistedSessions || {}));

  this.dispatchEvent(new shaka.util.FakeEvent(
      /* name= */ 'onplayerinit',
      /* data= */ {'manifest': this.manifest_, 'config': this.config_.drm}));

  console.log('[player] initializing player with', Object.assign({}, this.config_.drm));
  console.log('[player] ids', this.manifest_.offlineSessionIds.slice());
  console.log('[player] sessions', new Map(this.manifest_.persistedSessions));
